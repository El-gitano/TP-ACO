...
public class TestsIntegrationv2 {
	...
	public void insertionSuppressionRejeu() {
		...
		new Selectionner(moteur, new Selection(0, 8)).executer();
		new SupprimerTexte(moteur).executer();
		new InsererTexte(moteur, "Youp").executer();
		...
		//RàZ et ajout d'un nouveau texte pour le rejeu
		new Selectionner(moteur, new Selection(0, 14)).executer();
		new SupprimerTexte(moteur).executer();
		assertEquals("", ihm.getDerniereInsert());
		new InsererTexte(moteur, "Hello").executer();
		enregistreur.rejouerCommandes();
